...
public abstract class IMDB {
    ...
    /**
     * For a specific genre, get the num top rated movies.
     * The collection returned should have the movies for the specific genre ordered
     * by descending rating, and using the movie title alphabetically to resolve ties.
     *
     * @param num number of top movies
     * @param type the movie type, e.g. "MOVIE", "TV_SHOW", etc.
     * @param genre the genre, e.g. "Crime", "Drama", etc.
     * @return the map is keyed by genre, and the value
     * for the specific genre are the movies ordered by descending rating, using the movie
     * title alphabetically to resolve ties.
     */
    public abstract Map<String, List<Movie>> getMoviesTopRatedByGenre(int num, String type, String genre);
    ...
    public IMDB(boolean small) throws FileNotFoundException {
        ...
        while (in.hasNext()) {
            ...
            if (!fields[4].equals(IS_ADULT)) {     // ignore adult movies
                Movie movie = Movie.createMovie(fields[0], fields[1], fields[2], fields[5], fields[7], fields[8]); // create the Movie for this line using the appropriate fields
                this.movieList.add(movie); // append the Movie to the back of this.movieList
            }
        }
        in.close();
    ...
    public void processRatings() throws FileNotFoundException {
        ...
        while (in.hasNext()) {
            ...
            if (movieMap.get(fields[0]) != null) {
                movieMap.get(fields[0]).setRating(rating);
            }
        }
        in.close();
